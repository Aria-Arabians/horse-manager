...
function crawlList(event, ui) {
	...
	let listOfAttachments = jQuery('.attach_list li');
    let listToPost = [];
...
function saveFormData(formid) {
	...
	jQuery.post(ajaxurl, data, function(response) {
        ...
        jQuery('.img-overlay:first')
            .on("mouseover", overAttach)
        ...
        jQuery('.img-overlay:first .attach-buttons a')
            .on("mouseover", overEditButton )
		...
		jQuery('#img_li_'+responseObject.attach_id)
            .animate({width: '150px'}, 250, function() {
...
jQuery(document).ready(function() {
    ...
    jQuery('#menu-media')
        .before('<li><a href="" id="settings-divider" class="wp-has-submenu wp-not-current-submenu menu-top menu-icon-media menu-top-first" aria-haspopup="true"><div class="wp-menu-name">Advanced Settings</div></a></li>');
...
function selectParentId(id,title) {
    let parent_row = activeField.name[0]
    let node = activeField.name;

	parents_ajax_call(id, parent_row, node, title);
}

function parents_ajax_call(child_id, parent_row, node, title) {
    let node_depth = node.length;
    let node_cat = node[(node.length-1)];
