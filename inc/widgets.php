...
class horse_Widget_Searchform extends WP_Widget {
	...
	function horse_Widget_Searchform() {
		$widget_ops = array(
			'classname' => 'horse_Widget_Searchform',
			'description' => __( 'Let visitors search for horses in your site','wp_horse')
		);
...
class horse_Widget_Display extends WP_Widget {
	...
	function horse_Widget_Display() {
		parent::WP_Widget(
			...
			$widget_options = array(
				'name' => __('horse Display', 'wp_horse'),
				'description' => _x('Display horses by category or status, recent horses, etc','widget horse','wp_horse')
			)
		);
	}
	...
	function widget($args, $instance) {
		...
		<?php if ( $title ) echo $before_title . $title . $after_title; ?>
			...
			<?php if ( $r ) {  ?>
			<span>
				<a href="<?php echo home_url(); ?>/?feed=rss2&amp;post_type=horse">
				<span class="horse_rss">RSS</span>
				</a>
			</span>
			<?php } ?>
			...
			<?php  while ($q->have_posts()) : $q->the_post(); ?>
				<ul class="widget horse_info">
					<li>
						<span class="horse_image">
							<a href="<?php the_permalink() ?>"
									title="<?php the_title(); ?>">
									<?php the_post_thumbnail('horse_img'); ?>
							</a>
						</span>
					</li>
					<li class="horse_name">
					...
					<li class="horse_category">
						<?php $category = wp_get_object_terms(get_the_ID(), 'horse-category') ; echo '<span>'.__('In', 'wp_horse') . '</span> '. $category[0]->name ; ?>
					</li>
					<li class="horse_gender">
						<?php $gender = wp_get_object_terms(get_the_ID(), 'horse-gender') ; echo '<span>' . __('Gender', 'wp_horse') . '</span> '. $gender[0]->name ; ?>
					</li>
					<li class="horse_age">
						<?php $age = wp_get_object_terms(get_the_ID(), 'horse-age') ; echo '<span>' . __('Age', 'wp_horse') . '</span> '. $age[0]->name ; ?>
					</li>
					<li class="horse_size">
						<?php $size = wp_get_object_terms(get_the_ID(), 'horse-size') ; echo '<span>' . __('Size', 'wp_horse') . '</span> '. $size[0]->name ; ?>
					</li>
					<li class="horse_btn">
						<a href="<?php the_permalink() ?>"
								title="<?php _e('Read about', 'wp_horse'); ?>
								<?php the_title(); ?>">
								<span class="icon <?php $status = wp_get_object_terms(get_the_ID(), 'horse-status') ; echo $status[0]->slug ; ?>"></span>
								<?php $pd=get_the_ID(); $taxo = wp_get_object_terms($pd, 'horse-status') ; echo $taxo[0]->name ; ?>
						</a>
					</li>
				</ul>
				<div class="clear"></div>
	...
	?>
			...
			<p>
				...
				<select
					class="widefat" id="<?php echo $this->get_field_id('horse-category'); ?>"
					name="<?php echo $this->get_field_name('horse-category'); ?>">
			...
			<p>
				...
				<select
					class="widefat"
					id="<?php echo $this->get_field_id('horse-status'); ?>"
					name="<?php echo $this->get_field_name('horse-status'); ?>">
				<option value=""><?php _e('All','wp_horse'); ?></option>
				<?php
	...
	<input
		...
		name="<?php echo $this->get_field_name('rss'); ?>"<?php checked( $rss ); ?> />
